...
CREATE TABLE `ecell`.`tb_schedule` (
  ...
  `format` INTEGER UNSIGNED NOT NULL COMMENT '赛制',
...
CREATE TABLE `ecell`.`tb_match` (
  `id` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `season_id` INTEGER UNSIGNED NOT NULL COMMENT '赛季ID',
  `schedule_id` INTEGER UNSIGNED NOT NULL COMMENT '赛程ID',
  ...
  `pb_race` VARCHAR(45) COMMENT 'B选手种族',
  `duration` VARCHAR(45) COMMENT '比赛时长',
  `winner` INTEGER UNSIGNED COMMENT '胜利方',
  PRIMARY KEY (`id`)
)
ENGINE = InnoDB;
...
CREATE TABLE `ecell`.`tb_match_detail` (
  ...
  `player_race` VARCHAR(45) COMMENT '选手种族',
